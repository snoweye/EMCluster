/* Finding zeros of a function:
   Modified from R' source code (version 2.6.0). Please see at the end of these
   comments for copyright information.

   Modification Date: 1008/03/05

   Changes: - removed headers from for R_ext/Applic.h (and made standalone) 
            - renamed function to findzero to avoid future potential conflicts 
	      in common calls to R.

   This program is modified from the source code for the R statistical software
   package, simply so that we can find zeros in functionsm. The modification 
   is minor and listed above. 
...
double findzero2(double ax, double bx,	double fa, double fb, 
		 double (*f)(double x, void *info), void *info, double *Tol, 
		 int *Maxit);

double findzero(			/* An estimate of the root */
	...
	int *Maxit)				/* Max # of iterations */
{
	double fa = (*f)(ax, info);
	double fb = (*f)(bx, info);
	return findzero2(ax, bx, fa, fb, f, info, Tol, Maxit);
}

/* findzero2 or R_zeroin2() is faster for "expensive" f(), in those typical 
   cases where  f(ax) and f(bx) are available anyway : */

double findzero2(			/* An estimate of the root */
    double ax,				/* Left border | of the range	*/
    double bx,				/* Right border| the root is seeked*/
